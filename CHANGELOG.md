...
- Uses an opaque data-structure for `props.children`
...
- Adds top-level API `#Children...` for dealing with `props.children` opaques data-strucuture.
- Adds invarient errors for `render` if target container is invalid.

### Breaking Changes.

- `componentDidThrow` is renamed to `componentDidCatch` and unmounts the DOM if an error state is left unhandled. However errors coming from event handlers are exluded from this unmount behaviour.

- `props.children` is now an opaque data-structure, which might break your application if you relied on it always being an `Array`.
...
- Hydration is less forgiving in the sense that it will try to always make sure the DOM matches the virtual elements generated by components. This may result in removing nodes that do exist in the DOM tree but remain absent of the virual representation and vice-versa or adding new ones and vice-versa. This would be considered an improved behaviour, but if you relied on this, your application might break.
