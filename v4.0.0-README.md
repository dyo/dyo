...
When called this method will return an object with `routes`, `back`, `foward`, `link`, `resume`, `pause`, `destroy`, `set` and `location` properties. The location property can be used to navigate between views `router.location = '/user'` or link it to an action with `router.link` in the following way.
