...
The animation method defines what animations the component can use internally, once defined you could call the animation method with the name of the animation you want to execute and an optional callback as the second
argument - this returns a function you can call or attach as an event handler.

When called if no arguments are supplied it defaults to acting on the component node if an `Event` object is passed it defaults to currentTarget/target and if a `Node` is passed it defaults to act on that. 

Callbacks are called with the component as the context.
...
class Hello extends dio.Component {
	...
	componentWillMount () {
		setTimeout(this.animation('componentWillMount'))
	}
	handleClick () {
		// ...
	}
	render () {
		return h('div', {onClick: this.animation('onClick', this.handleClick)})
	}
}
```
...
getDefaultProps is a method used to assign the default props of a component when the component does not received props. This method receives no arguments and returns `{(undefined|null|Object)}`.
